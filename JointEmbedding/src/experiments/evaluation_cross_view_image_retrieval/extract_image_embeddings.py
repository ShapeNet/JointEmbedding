...
datasets = ['02958343', '03001627_clutter', '03001627_clean']

for dataset in datasets: 
    input_data = [None]*batch_size
    filelist = [line.strip() for line in open(os.path.join(evaluation_folder, 'filelist_'+dataset+'.txt'))]
    print datetime.datetime.now().time(), 'Computing image embedding for %s (%d images)...'%(dataset, len(filelist)) 
    
    data_folder = os.path.join(evaluation_folder, 'images_'+dataset)
    image_embedding_filename = os.path.join(evaluation_folder, 'image_embeddings_'+dataset+'.txt')
